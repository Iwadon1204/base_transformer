...
class TransformerEmbeddingBlock(nn.Module):
    ...
    def __init__(self, vocab_size: int, model_dim: int, input_length: int, drop_prob: float, device: str):
        """
        Transformerにおける埋め込みブロックを初期化します
        :param int vocab_size: 語彙数
        :param int model_dim: モデルの次元数
        :param int input_length: 入力長
        ...
        # 位置埋め込みレイヤーの初期化
        self.l_positional_encoding = PostionalEncodingLayer(model_dim, input_length, device)
        # ドロップアウト
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        :param x: 単語ID列(Tensor)
        :return:
        """
        # 埋め込み表現を獲得[batch * max_length * d_model]
        out_token_emb = self.l_token_embedding(x)
        # 位置エンコーディングを取得。本処理を行うことでトークン列の位置情報を埋め込みます
